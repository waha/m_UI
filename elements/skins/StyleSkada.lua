...
--		skada.backdrop:SetBackdrop(nil)
--		skada.backdrop:SetBackdropBorderColor(nil)
...
local function EmbedWindow(window, width, height, max, point, relativeFrame, relativePoint, ofsx, ofsy)
	...
	window.db.background.height = 123 --123
...
function EmbedSkada()
	if #windows == 1 then
		EmbedWindow(windows[1], 160,13, 9, "BOTTOMRIGHT", "UIParent", "BOTTOMRIGHT", -169, 34)
	end
end
